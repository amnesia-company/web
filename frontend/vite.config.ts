...
function readDefaultLanguage(): string {
  const content = fs.readFileSync(path.resolve(__dirname, 'src/shared/const/const.ts'), 'utf-8');
  ...
  const urls = routes.flatMap((route) =>
    languages.map((lang) => {
      ...
      const alternates = languages
        .map(
          (l) =>
            `        <xhtml:link rel="alternate" hreflang="${l}" href="${siteUrl}${route}?lang=${l}" />`
        )
        .join('\n');
...
export default defineConfig({
  plugins: [react(), babel({ presets: [reactCompilerPreset()] }), vike({}), sitemapPlugin()],
  resolve: {
    alias: {
